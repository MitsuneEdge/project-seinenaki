...
if (typeof window !== 'undefined') {
    ...
    trigger.L.addEventListener('click', async (e) => {
        // 阻止事件冒泡（避免立即触发关闭）
        e.stopPropagation();
        content.L.innerHTML = await loadContent('/src/content/OverlayWindow/indexContent-L1.html');
        content.R.innerHTML = await loadContent('/src/content/OverlayWindow/indexContent-L2.html');
    ...
    trigger.R.addEventListener('click', async (e) => {
        // 阻止事件冒泡（避免立即触发关闭）
        e.stopPropagation();
        content.L.innerHTML = await loadContent('/src/content/OverlayWindow/indexContent-R1.html');
